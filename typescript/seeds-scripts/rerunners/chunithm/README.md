...
```shell
bun merge-options.ts --version <VERSION> --input /path/to/chunithm/App/data /path/to/chunithm/Option
```

2. In the same directory, reorder the versions by running `bun sort-versions.js`.
...
```
bun parse-song-duration.ts -v "$(which vgmstream-cli)" -d /path/to/chunithm/App/data -g chunithm
bun parse-song-duration.ts -v "$(which vgmstream-cli)" -d /path/to/chunithm/Option -g chunithm
```
...
```
bun toggle-table-inactive.js -t chunithm-Single-<VERSION>-levels
```
