...
const splitTitle = (song: OngekiSong) => {
	...
	const [, title, rawCharacter] = match;
	return {
		title: title!.trim(),
...
const main = () => {
	...
	for (const bonusSong of songs) {
		...
		const { title, rawCharacter } = splitTitle(bonusSong);
		const originalSong = findOriginalSong(title, songs);

		bonusSong.data.flavorGenre = originalSong.data.flavorGenre;

		if (!(rawCharacter in characters)) {
			throw new Error(`Unknown character: ${rawCharacter}`);
		}

		const character = rawCharacter as keyof typeof characters;
