...
/** Discord allows at most 25 string choices per option. */
export const SELF_ASSIGN_ROLE_CHOICE_LIMIT = 25;
...
export function selfAssignableSlashChoiceKeys(
	...
	const sorted = (Object.keys(map) as GameGroup[]).sort((a, b) => a.localeCompare(b));
	const keysIncluded = sorted.slice(0, limit);
	const keysOmitted = sorted.slice(limit);
...
/** Returns slash command `/roles`, or null if there is nothing to expose. */
export function getRolesSlashCommand(): SlashCommand | null {
	...
	if (keysOmitted.length > 0) {
		log.warn(
			...
			`DISCORD_GAME_ROLES exposes more GameGroups than Discord's slash choice limit; only the first ${SELF_ASSIGN_ROLE_CHOICE_LIMIT} (alphabetical) appear in \`/roles\`.`,
		);
	}
	...
	const command: SlashCommand = {
		info: new SlashCommandBuilder()
			.setName("roles")
			.setDescription("Add or remove self-assignable Discord roles.")
			.addSubcommand((sub) =>
				attachRoleChoiceOption(
					sub.setName("add").setDescription("Grant yourself one of these roles."),
					choiceTuples,
				),
			)
			.addSubcommand((sub) =>
				attachRoleChoiceOption(
					sub
						.setName("remove")
						.setDescription("Remove one of these roles from yourself."),
					choiceTuples,
				),
			)
			.toJSON(),
